...
iou_thres = 0.7  # IoU 阈值，用于非极大值抑制（NMS）
agnostic_nms = False  # 是否使用无类别的 NMS
classes = None  # 类别索引列表（None 表示检测所有类别）
augment = False  # 是否在推理时使用数据增强
set_logging()  # 设置日志记录
device = select_device(0)  # 选择设备（0 表示 GPU，如果可用；否则使用 CPU）
model = attempt_load(weights)  # 加载 YOLOv5 模型权重
stride = int(model.stride.max())  # 获取模型的最大步幅
imgsz = check_img_size(imgsz, s=stride)  # 检查输入图像大小是否符合模型要求
model.float().to(device)  # 将模型转换为 float32 类型并移动到指定设备
...
# 相机内参和手眼标定矩阵
fcamera_matrix = np.array([[582.13431296, 0, 316.00723293], 
                           [0, 582.99623535, 268.45817221], 
                           [0, 0, 1]])
frotation_matrix = np.array([[0.01396891, 0.99845847, 0.04233127],
                            [0.99925939, 0.03645072, -0.01232946],
                           [-0.01385346, 0.0418573, -0.99902755]])
ftranslation_vector = np.array([[-51.52974737], [-28.13069681], [193.99693148]])
fhand_eye_matrix = np.eye(4)
fhand_eye_matrix[:3, :3] = frotation_matrix
fhand_eye_matrix[:3, 3] = ftranslation_vector.flatten()

camera_matrix = np.array([[569.60940903, 0, 319.00747461],
                         [0, 569.7973498, 236.88064889], 
                         [0, 0, 1]])
rotation_matrix = np.array([[-0.00310148, 0.99806818, -0.06205076], 
                            [0.99985684, 0.00412721, 0.01640924], 
                            [0.01663363, -0.06199098, -0.99793809]])
translation_vector = np.array([[-9.96500977], [-45.99854704], [170.172746]])
...
shared_frame = np.zeros((480, 640, 3), dtype=np.uint8)
...
def foodcenter(detections, depth_image, frame, tracker, camera_matrix, hand_eye_matrix):
    ...
    try:
        with tracker_lock:
            ...
            print("Tracked objects:", tracked_objects)  # 调试信息
            ...
            for trk in tracked_objects:
                ...
                print(f"Depth value at ({x_center}, {y_center}): {depth_value}")  # 调试信息

                if depth_value == 0:
                    print("Invalid depth value, skipping.")
                    continue
                ...
                print("Target coordinates:", target_coords)  # 调试信息

                return target_coords

    except Exception as e:
        print(f"Error in foodcenter: {e}")
        return None
