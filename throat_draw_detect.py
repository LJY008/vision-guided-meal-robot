...
# 使用第一个设备
serial_number = device_list[0]  # 修改这里以选择不同的设备
...
class TrackerData:
    def __init__(self, window_size=300, num_frames=5):
    ...
    def monitor_chewing_swallowing(self, displacements):
        last_change_time = {0: None, 1: None}
        last_displacement = {0: 0, 1: 0}
        ...
        for track_id, disp in displacements.items():
            ...
            if latest_0 != last_displacement[0] and latest_1 != last_displacement[1]:
                if (latest_0 > 0 and latest_1 > 0) or (latest_0 < 0 and latest_1 < 0):
                    print("Chewing detected")

            if last_change_time[1] is not None and last_change_time[0] is not None:
                if latest_1 == 0 and latest_0 == 0 and last_change_time[1] < last_change_time[0]:
                    print("Swallowing detected")

            last_displacement[0] = latest_0
            last_displacement[1] = latest_1
            last_change_time[0] = time.time() if latest_0 != 0 else last_change_time[0]
            last_change_time[1] = time.time() if latest_1 != 0 else last_change_time[1]
