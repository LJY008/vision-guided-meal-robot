...
fcamera_matrix = np.array([
    [601.86522231, 0, 305.65401678],
    [0, 600.2412298, 248.03488758],
    [0, 0, 1]
])

# 手眼标定结果
frotation_matrix = np.array([
    [0.01396891, 0.99974462, -0.01776415],
    [0.99989052, -0.01405323, -0.00463094],
    [-0.0048794, -0.01769751, -0.99983148]
])
...
camera_matrix = np.array([
    [594.89438765, 0, 312.23623684],
    [0, 595.45647678, 267.77101126],
    [0, 0, 1]
])

# 手眼标定结果
rotation_matrix = np.array([
    [0.02144892, 0.99968928, 0.01269976],
    [0.99967071, -0.02126609, -0.01436061],
    [-0.01408607, 0.0130036, -0.99981623]
])
...
joints =joints = [-0.6789181783265179, -0.45611943983119424, 0.7721917808314761, 2.6023867647909382, -1.5208100281084818, -0.5678124226065379, -0.9311741433949958]
DianaApi.setPushPeriod(10, ipAddress)
# 工具坐标系设置
# matrix = (-28.8008,4.77148,-226.509,0.0,0.0,0.0)
# DianaApi.setDefaultActiveTcpPose(matrix,' 192.168.10.75')
# 设置 YOLOv5 参数
weights = 'yolov5/runs/train/food_custom_train5/weights/best.pt'  # 训练好的权重文件路径
imgsz = 640  # 图像大小
...
def process_frames(stop_event):
    global velocity, acceleration, detect_food_flag, detect_face_flag, target_coords, food, prepare_get_completed,place_food_completed
    ...
    while not stop_event.is_set():
        ...
        if detect_face_flag:
            ...
            detections, detected_image = detect(frame)
            new_food = foodcenter(detections, depth_image, frame, tracker, fcamera_matrix, fhand_eye_matrix)
    ...
    while not stop_event.is_set():
        ...
        if feeding_in_progress:
            ...
            if current_target is not None:
                place_food(current_target, 'fork')
        ...
        time.sleep(0.05)  # 调整机器人动作检查频率以提高响应速度
